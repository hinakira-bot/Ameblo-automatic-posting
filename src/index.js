...
async function getTestLogin() {
  validateConfig();
  const { testLogin } = await import('./ameblo-poster.js');
  return testLogin;
}
...
program
  .name('ameblo-auto-poster')
  .description('アメブロ自動投稿ツール - Gemini AI + SEO分析')
  .version('2.0.0');
...
// === test-login: アメブロログインテスト ===
program
  .command('test-login')
  .description('アメブロへのログインをテスト（ブラウザが開きます）')
  .action(async () => {
    console.log('\n🔍 アメブロ ログインテスト中...\n');
    ...
    if (result) {
      ...
      console.log('❌ ログイン失敗 - .env の AMEBLO_ID / AMEBLO_PASSWORD を確認してください');
    }
