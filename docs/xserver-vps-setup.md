...
アメブロ自動投稿ツールを Xserver VPS で動かすための完全手順です。
...
# sudo権限を付与
usermod -aG sudo ameblo

# SSH鍵をコピー
mkdir -p /home/ameblo/.ssh
cp ~/.ssh/authorized_keys /home/ameblo/.ssh/
chown -R ameblo:ameblo /home/ameblo/.ssh
chmod 700 /home/ameblo/.ssh
chmod 600 /home/ameblo/.ssh/authorized_keys
...
Playwright（ブラウザ自動操作）に必要なシステムライブラリをインストール：
...
```bash
# /opt にデプロイ
sudo mkdir -p /opt/ameblo-tool
sudo chown ameblo:ameblo /opt/ameblo-tool
...
3. **アメブロID・パスワード入力** → アメブロのログイン情報を入力
4. **完了** → ダッシュボードへ
...
```bash
sudo nano /etc/nginx/sites-available/ameblo-tool
```
...
# 直近のエラーのみ
pm2 logs ameblo-tool --err --lines 50
