#!/bin/bash
# ============================================
# アメブロ自動投稿ツール - ワンクリックインストーラー
# 使い方: curl -fsSL https://raw.githubusercontent.com/hinakira-bot/Ameblo-automatic-posting/main/install.sh | bash
# ============================================
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  アメブロ自動投稿ツール インストーラー${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ -d "$INSTALL_DIR" ]; then
    ...
    git clone https://github.com/hinakira-bot/Ameblo-automatic-posting.git "$INSTALL_DIR"
    cd "$INSTALL_DIR"
fi
...
if [ ! -f .env ]; then
    ...
    read -p "アメブロID: " AMEBLO_ID
    read -p "アメブロパスワード: " AMEBLO_PASS
...
# --- データディレクトリ作成 ---
mkdir -p data logs images knowledge data/session
